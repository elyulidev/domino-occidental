...
class Board {
  ...
  print() {
    const keys = Object.keys(this.rows).sort(
      (a, b) => Number(b.slice(1)) - Number(a.slice(1)),
    );
    for (const k of keys) {
      const cells = this.rows[k]
        .map((c, _i) => (c !== null ? `${c}`.padStart(2, " ") : " ."))
        .join("");
...
function filterExt(board, ext) {
  board.openEnds = board.openEnds.filter(
    (e) => !(e.row === ext.row && e.col === ext.col && e.dir === ext.dir),
  );
}
...
function _doubleFromGiro(board, val, ext, _step) {
  const { row, col, side } = ext;
  ...
  return {
    ok: true,
    act: `DOB-VERT ${row}:C${col}\u2192${nr1}:C${col}`,
    tag: "dob-v0",
  };
}
...
function _double(board, val, ext, step, space) {
  ...
  if (space === 0) {
    ...
    return {
      ok: true,
      act: `L-CORNER-DOB ${row}:C${col}\u2192${nr1}:C${col}\u2192${nr2}:C${col}`,
      tag: "dob-L0",
    };
  }
  const nc = col + step;
...
function _newRow(board, cv, fv, ext) {
  ...
  filterExt(board, ext);
  setNewEnds(board, [
    { val: fv, row: nr, col: c_borde, dir: nd, side, giroExtreme: true },
  ]);
  return {
    ok: true,
    act: `${row}:C${c_borde}\u2192${nr}:C${c_borde}`,
    tag: "=1",
  };
}

function _lcornerFromGiro(board, _cv, fv, ext, _step) {
  const { row, col, side } = ext;
  const vertDir = board.decideVerticalDir(side);
  const ri = Number(row.slice(1));
  const off1 = vertDir === "up" ? 1 : -1;
  ...
  return {
    ok: true,
    act: `VERT-DIR ${row}:C${col}\u2192${nr1}:C${col}`,
    tag: "=0-v",
  };
}

function _lcorner(board, cv, fv, ext, _step) {
  const { row, col, side } = ext;
  const vertDir = board.decideVerticalDir(side);
  const ri = Number(row.slice(1));
  const off1 = vertDir === "up" ? 1 : -1;
  ...
  return {
    ok: true,
    act: `L-CORNER ${row}:C${col}\u2192${nr1}:C${col}\u2192${nr2}:C${col}`,
    tag: "=0",
  };
}
...
for (let i = 0; i < SEQUENCE.length; i++) {
  ...
  if (!cand.length) {
    ...
    for (const e of B.openEnds) {
      console.log(`        ${e.val} -> ${e.row}:C${e.col}(${e.dir})`);
    }
    break;
  }
  ...
  const ext = cand.sort(
    (a, b) =>
      B.countFree(b.row, b.col, b.dir) - B.countFree(a.row, a.col, a.dir),
  )[0];
  const connVal = ext.val === a ? a : b;
  const _freeVal = connVal === a ? b : a;
  const dbl = a === b;
  const space = B.countFree(ext.row, ext.col, ext.dir);
  const _step = ext.dir === "\u2192" ? 1 : -1;

  const routeInfo = [];
  if (dbl && space === 0 && ext.giroExtreme)
    routeInfo.push("dbl+space=0+giroExtreme → _doubleFromGiro");
  else if (dbl && space === 0)
    routeInfo.push("dbl+space=0 → _double(L-CORNER-DOB)");
  else if (dbl && space === 1) routeInfo.push("dbl+space=1 → _newRow");
  else if (dbl && space >= 2 && _isNewRow(B, ext.row))
    routeInfo.push("dbl+space>=2+_isNewRow → _horiz(spinner)");
    ...
    routeInfo.push("space=0+giroExtreme → _lcornerFromGiro");
...
B.openEnds.forEach((e) => {
  ...
  console.log(
    `  ${e.val} en ${e.row}:C${e.col}(${e.dir}) space=${space} atEdge=${atEdge} side=${e.side}`,
  );
});
