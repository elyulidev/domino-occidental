...
for (let run = 1; run <= 8; run++) {
  ...
  for (let i = tableStart + 2; i < (tableEnd > tableStart ? tableEnd : lines.length); i++) {
  ...
  const positiveRows = rows.filter(r => r.startsWith('F') && !r.startsWith('F-') && r !== 'F0' && r !== 'F1');
  ...
  console.log(`Run ${run}: ${filasMatch ? 'Filas=' + filasMatch[0] : '?'} | Rows: ${rows.join(',')}`);
  if (ghosts.length > 0) {
    console.log(`  👻 GHOST ROWS: ${ghosts.join(', ')} (exist alongside their negative counterparts)`);
  }
}
