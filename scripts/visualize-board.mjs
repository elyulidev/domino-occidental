...
for (const r of sortedRows) {
  ...
  const vals = cells
    .map((c, _i) => (c !== null ? String(c).padStart(2, " ") : "··"))
    .join("  ");
...
console.log(
  "   #  Ficha     Ori  Posición fichas                Caso              Espacio",
);
console.log(
  "  ────────────────────────────────────────────────────────────────────────────────",
);
board.seq.forEach((s, i) => {
  ...
  const tile = `[${s.a}|${s.b}]`;
  console.log(
    `  ${String(n).padStart(2)}  ${tile.padEnd(7)} ${dir}  ${s.act.padEnd(31)} ${tagLabel.padEnd(16)} space=${s.space}`,
  );
});
console.log(
  "  ────────────────────────────────────────────────────────────────────────────────",
...
console.log(
  "  FICHAS [a|b] EN POSICIÓN (lectura: cada ficha está donde fue colocada)\n",
);
...
for (const s of board.seq) {
  ...
  if (s.tag === ">=2") {
    ...
    tilesByRow[s.extRow].push({
      tile: tileStr,
      col: `C${Math.min(c1, c2)}-C${Math.max(c1, c2)}`,
      isDouble: false,
      tag: "H",
    });
  } else if (s.tag === "=1") {
...
console.log("  ────────────────────────────────────────────────────");
console.log(
  `  Fichas colocadas:       ${board.playedSet.size}/55 del set doble-9`,
);
...
console.log(
  `  Misma fila (≥2):        ${seqTags.filter((t) => t === ">=2").length}`,
);
console.log(
  `  Giro vertical (=1):     ${seqTags.filter((t) => t === "=1").length}`,
);
console.log(
  `  L-Corner (=0):         ${seqTags.filter((t) => t === "=0").length}`,
);
const uniqueCheck = new Set(
  board.seq.map((s) => `${Math.min(s.a, s.b)}-${Math.max(s.a, s.b)}`),
);
