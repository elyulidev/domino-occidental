...
function showGrid(
  ...
  grid: ReturnType<typeof computeGridLayout>,
) {
  console.log(`\n${table}`);
  ...
  for (const t of tiles) {
    ...
    if (!gt) {
      console.log(`  ${t.label}: NO ENCONTRADO`);
      continue;
    }
    const c = gt.cells;
    ...
    console.log(`  ${t.label}`);
    console.log(
      `    cells: F${c[0].row}:C${c[0].col}(${c[0].value}) -- F${c[1].row}:C${c[1].col}(${c[1].value})  ${center}`,
    );
    console.log(`    ${orient}${info}`);
    if (gt.isDouble && gt.orientation === "horizontal") {
      console.log(
        `    ** DOBLE HORIZONTAL en newRow — falta spinner end opuesto **`,
      );
    }
  }
  ...
  for (const r of allRows) {
    ...
    for (let c = 0; c < 16; c++) {
      ...
      line +=
        val !== undefined ? ` ${String(val).padEnd(cellW - 1, " ")}` : " .  ";
    }
    console.log(line);
  }
  ...
  console.log(`\nHeads:`);
  console.log(
    `  left:  F${grid.leftHead?.row}:C${grid.leftHead?.col} dir=${grid.leftHead?.dir} giro=${grid.leftHead?.giroExtreme ?? false}`,
  );
  console.log(
    `  right: F${grid.rightHead?.row}:C${grid.rightHead?.col} dir=${grid.rightHead?.dir} giro=${grid.rightHead?.giroExtreme ?? false}`,
  );
}
...
for (const id of ["t4", "t5", "t6"]) {
  // biome-ignore lint/style/noNonNullAssertion: script de debug, find() siempre encuentra
  ...
  const info =
    gt.isDouble && gt.orientation === "vertical" ? " (doble con floats)" : "";
  console.log(
    `  ${id}: (${x}, ${y}) w=${w} h=${h}${info}  →  rows ${y - h / 2}..${y + h / 2}, cols ${x - w / 2}..${x + w / 2}`,
  );
}

// Check adjacency
// biome-ignore lint/style/noNonNullAssertion: script de debug
...
console.log(
  `  t4 right cell = F${t4.cells[1].row}:C${t4.cells[1].col}(${t4.cells[1].value})`,
);
console.log(
  `  t5 cells[0]  = F${t5.cells[0].row}:C${t5.cells[0].col}(${t5.cells[0].value})`,
);
console.log(
  `  Match: ${t4.cells[1].value === t5.cells[0].value ? "✓ MISMO VALOR" : "✗ DIFERENTE"}`,
);
console.log(
  `  t5 cells[1]  = F${t5.cells[1].row}:C${t5.cells[1].col}(${t5.cells[1].value})`,
);
console.log(
  `  t6 cells[0]  = F${t6.cells[0].row}:C${t6.cells[0].col}(${t6.cells[0].value})`,
);
console.log(
  `  Match: ${t5.cells[1].value === t6.cells[0].value ? "✓ MISMO VALOR" : "✗ DIFERENTE"}`,
);
...
console.log(
  `  cells: F${t6s2.cells[0].row}:C${t6s2.cells[0].col} -- F${t6s2.cells[1].row}:C${t6s2.cells[1].col}`,
);
console.log(`  floats: ${t6s2.floats.length}`);
if (t6s2.floats.length > 0) {
  console.log(
    `  float cells: F${t6s2.floats[0].row}:C${t6s2.floats[0].col}, F${t6s2.floats[1].row}:C${t6s2.floats[1].col}`,
  );
}
console.log(
  `  Ocupa 1 celda x ${t6s2.orientation === "horizontal" ? "2 celdas (HORIZ)" : t6s2.isDouble ? "3 celdas (VERT con floats)" : "2 celdas (VERT sin floats)"}`,
);
