...
function autoPlay(board) {
  ...
  while (board.playedSet.size < 40) {
    ...
    if (r.ok) {
      ...
      stuck++;
      if (stuck > 3) {
        console.log(`  STUCK en #${board.playedSet.size + 1}`);
        return false;
      }
    }
  }
...
B.seq.forEach((s, i) => {
  ...
  const conn = `${s.connVal}->${s.extRow}:C${s.extCol}(${s.extDir})`.padEnd(
    22,
    " ",
  );
  const tagMap = {
...
for (const e of B.openEnds) {
  console.log(`  ${e.val} -> ${e.row}:C${e.col}(${e.dir}) side=${e.side}`);
}
...
console.log(
  `  L-Corners: ${B.seq.filter((s) => s.tag === "=0" || s.tag === "dob-L0").length}`,
);
console.log(
  `  Nuevas filas (space=1): ${B.seq.filter((s) => s.tag === "=1").length}`,
);
console.log(
  `  Vert-Dir (pos-giro): ${B.seq.filter((s) => s.tag === "=0-v" || s.tag === "dob-v0").length}`,
);
console.log(
  `  Misma fila (space>=2): ${B.seq.filter((s) => s.tag === "\u22652").length}`,
);
...
for (const s of doblesSeq) {
  console.log(
    `  #${B.seq.indexOf(s) + 1} [${s.a}|${s.b}] en ${s.extRow}:C${s.extCol}(${s.extDir}) — ${s.act}`,
  );
}
