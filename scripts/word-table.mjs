...
class Board {
  ...
  countFree(r, c, d) { let col = c + (d === '→' ? 1 : -1); let n = 0; while (col >= 0 && col < COLS && this.getCell(r, col) === null) { n++; col += (d === '→' ? 1 : -1); } return n; }
...
function place(board, A, B) {
  ...
  if (r.ok) { board.markPlayed(A, B); board.seq.push({ a: A, b: B, connVal, freeVal, extRow: row, extCol: col, extDir: dir, act: r.act, tag: r.tag, space }); }
  return r;
}
function _double(board, val, row, col, dir, step, space) {
  ...
  if (space === 0) {
    const nr = board.newRow(); board.setCell(nr, col, val);
    const nri = Number(nr.slice(1)); board.ensureRow(`F${nri - 1}`); board.ensureRow(`F${nri + 1}`);
...
if (!ok) { console.log('Falló'); process.exit(1); }
...
tileMap['F0'].push({ tile: '[9|9]', colStart: 7, colEnd: 7, a: 9, b: 9, isDouble: true, isVertical: false });
tileMap['F-1'] = tileMap['F-1'] || [];
tileMap['F-1'].push({ tile: '[9|9]', colStart: 7, colEnd: 7, a: 9, b: 9, isDouble: true, isVertical: false, float: true });
tileMap['F1'] = tileMap['F1'] || [];
tileMap['F1'].push({ tile: '[9|9]', colStart: 7, colEnd: 7, a: 9, b: 9, isDouble: true, isVertical: false, float: true });
...
for (const s of board.seq) {
  if (s.tag === '>=2') {
    ...
    tileMap[s.extRow].push({ tile: `[${s.connVal}|${s.freeVal}]`, colStart: minC, colEnd: maxC, a: s.connVal, b: s.freeVal, isDouble: false, isVertical: false });
  } else if (s.tag === '=1') {
    const c_borde = s.extCol + (s.extDir === '→' ? 1 : -1);
    const parts = s.act.split('->');
    const nr = parts[1].split(':')[0];
    tileMap[s.extRow] = tileMap[s.extRow] || [];
    tileMap[s.extRow].push({ tile: `[${s.connVal}|${s.freeVal}]`, colStart: c_borde, colEnd: c_borde, a: s.connVal, b: s.freeVal, isDouble: false, isVertical: true, role: 'conn' });
    tileMap[nr] = tileMap[nr] || [];
    tileMap[nr].push({ tile: `[${s.connVal}|${s.freeVal}]`, colStart: c_borde, colEnd: c_borde, a: s.connVal, b: s.freeVal, isDouble: false, isVertical: true, role: 'free' });
  } else if (s.tag === '=0') {
    const parts = s.act.split('->');
    const nr = parts[1].split(':')[0];
    tileMap[s.extRow] = tileMap[s.extRow] || [];
    tileMap[s.extRow].push({ tile: `[${s.connVal}|${s.freeVal}]`, colStart: s.extCol, colEnd: s.extCol, a: s.connVal, b: s.freeVal, isDouble: false, isVertical: true, role: 'conn' });
    tileMap[nr] = tileMap[nr] || [];
    tileMap[nr].push({ tile: `[${s.connVal}|${s.freeVal}]`, colStart: s.extCol, colEnd: s.extCol, a: s.connVal, b: s.freeVal, isDouble: false, isVertical: true, role: 'free' });
  }
}
...
console.log('  Algoritmo: algoritmo_mejorado.md — Giro Mixto Vertical corregido');
console.log('================================================================================');
console.log('');
...
// Header
console.log('+------+----------+-------+-----------------------------------+----------------+---------+');
console.log('|  #   | Ficha    | Dir   | Posicion                          | Caso           | Espacio |');
console.log('+------+----------+-------+-----------------------------------+----------------+---------+');
...
board.seq.forEach((s, i) => {
  ...
  console.log(`| ${String(n).padStart(2)}   | ${tile.padEnd(7)} | ${dir} | ${s.act.padEnd(33)} | ${paddedTag} | ${String(s.space).padStart(7)} |`);
});
console.log('+------+----------+-------+-----------------------------------+----------------+---------+');
console.log('');
...
console.log('Leyenda:  +---+---+  ficha horizontal 2 cols    +---+  ficha vertical/doble 1 col    |   |  conexion vertical');
console.log('');
...
for (const r of sortedRows) {
  ...
  console.log(`  ${r} (${dir}):${pad}${tiles.map(t => {
...
console.log(`  Dobles:              ${used.filter(([a, b]) => a === b).length}`);
console.log(`  Filas usadas:        ${sortedRows.length}`);
console.log(`  Misma fila:          ${seqTags.filter(t => t === '>=2').length}`);
console.log(`  Giro vertical:       ${seqTags.filter(t => t === '=1').length}`);
console.log(`  L-Corner:           ${seqTags.filter(t => t === '=0').length}`);
const dupes = board.seq.length - new Set(board.seq.map(s => `${Math.min(s.a,s.b)}-${Math.max(s.a,s.b)}`)).size;
console.log(`  Fichas repetidas:    ${dupes === 0 ? '0 (OK)' : dupes}`);
console.log('');
console.log('  Extremos abiertos:');
board.openEnds.forEach(e => console.log(`    ${e.val} en ${e.row}:C${e.col} (${e.dir})`));
console.log('');
