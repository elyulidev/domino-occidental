...
async function main() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    await new Promise((resolve, reject) => { ws.onopen = () => { console.log(pid + ' OK'); resolve(); }; ws.onerror = (e) => reject(e); setTimeout(() => reject('timeout'), 5000); });
    ws.onmessage = (e) => { const d = JSON.parse(e.data); if (d.state) ws.__s = d.state; };
    players.push(ws);
  }
