...
function MatchContent() {
  ...
  // Loading state
  if (view === "loading") {
    return <LoadingScreen wsStatus={wsHook.status} mode={mode} playerId={playerId} />;
  }
  ...
  return (
    <div className="relative min-h-screen bg-domino-950 text-domino-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_140px] gap-4 p-4 h-screen max-h-screen">
        {/* Left: Score Panel */}
...
function LoadingScreen({ wsStatus, mode, playerId }: { wsStatus?: WsStatus; mode?: string; playerId?: string }) {
  return (
    <div className="min-h-screen bg-domino-950 flex items-center justify-center">
      <div className="text-center max-w-sm">
        ...
        <div className="mt-6 space-y-1 text-xs font-mono flex flex-col items-center">
          {wsStatus === "connected" && <span className="inline-flex items-center gap-1.5 text-green-400"><span className="h-2 w-2 rounded-full bg-green-400" />WS: connected</span>}
          {wsStatus === "connecting" && <span className="inline-flex items-center gap-1.5 text-yellow-400"><span className="h-2 w-2 rounded-full bg-yellow-400" />WS: connecting</span>}
          {wsStatus === "disconnected" && <span className="inline-flex items-center gap-1.5 text-red-400"><span className="h-2 w-2 rounded-full bg-red-400" />WS: disconnected</span>}
          {!wsStatus && <span className="inline-flex items-center gap-1.5 text-domino-500"><span className="h-2 w-2 rounded-full bg-domino-500" />WS: —</span>}
        ...
        {wsStatus === "disconnected" && (
          <p className="mt-4 text-xs text-red-400/80 max-w-xs mx-auto">
            WebSocket is disconnected. Make sure the backend is running on port 3001 and refresh the page.
          </p>
        )}

        {wsStatus === "connected" && (
          <p className="mt-4 text-xs text-domino-400 max-w-xs mx-auto">
            WebSocket connected. Waiting for the game server to send the initial state.
          </p>
        )}
      </div>
    </div>
