...
#### Scenario: Handler records move on successful play

- GIVEN a valid `play_tile` message that changes match state
- WHEN `handleMessage` processes the action
- THEN `recordMatchMove` MUST be called with a `MoveRecord` containing tile data, board ends, and round number
- AND the move record MUST have `isPass: false`

#### Scenario: Handler records move on pass

- GIVEN a valid `pass` message that changes match state
- WHEN `handleMessage` processes the action
- THEN `recordMatchMove` MUST be called with a `MoveRecord` having `isPass: true` and no tile fields

#### Scenario: Handler does NOT record on leave

- GIVEN a `leave` (forfeit) message
- WHEN `handleMessage` processes the action
- THEN `recordMatchMove` MUST NOT be called

#### Scenario: Handler does NOT record on error

- GIVEN a `play_tile` or `pass` message that fails validation (NOT_YOUR_TURN, INVALID_PLAY)
- WHEN `handleMessage` processes the action
- THEN `recordMatchMove` MUST NOT be called
- AND match state MUST remain unchanged
