# lobby-server-fetch Specification

## Purpose

Server-side data fetching for the lobby page via Supabase direct queries. Replaces client-side `apiFetch` calls with an async server component pattern. All 8 lobby sections become props-driven pure functions defined in `page.tsx`.
